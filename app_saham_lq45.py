...
def plot_arima_residuals(model_fit):
...
def train_evaluate_auto_arima_plain(ts_data):
    from pmdarima import auto_arima
    def evaluate_arima(y_true, y_pred):
        mse = np.mean((y_true - y_pred)**2)
        rmse = np.sqrt(mse)
    ...
    model = auto_arima(
        ...
        d=None, seasonal=False, stepwise=True, suppress_warnings=True
    )
    ...
    mse, rmse, mae, mape = evaluate_arima(y_true, y_pred)

    return model, model.get_params()['order'], tanggal, y_true, y_pred, mse, rmse, mae, mape

def train_arima_full(ts_data):
    from pmdarima import auto_arima
    ts_data = ts_data.astype(float).dropna()
    model = auto_arima(
        ts_data, start_p=0, start_q=0, max_p=5, max_q=5,
        d=None, seasonal=False, stepwise=True, suppress_warnings=True
    )
    return model, model.get_params()['order']


# Fungsi evaluasi error
...
# Tuning model
def tune_glm_model(df_raw, max_lags=10):
    best_mse = float('inf')
    best_lags = 1
    best_model = None
...
# Halaman konfigurasi data
    ...
    try:
        ...
        # Format tanggal jadi string YYYY-MM-DD agar tidak tampil jam
        df_filtered["date"] = df_filtered["date"].dt.strftime('%Y-%m-%d')

        # --- Tampilkan data set yang digunakan ---
        st.subheader("Data Saham yang Digunakan")
        st.dataframe(df_filtered[["date", "open_price", "high", "low", "close", "volume"]])

    except FileNotFoundError:
        st.error(f"Data untuk simbol **{selected_symbol}** tidak ditemukan.")
    except Exception as e:
...
# Halaman evaluasi
    ...
    if model_type == "GLM":
        ...
        mse, rmse, mae, mape, r2, adj_r2 = evaluate_glm(y_true, y_pred)

        # Hasil Evaluasi GLM
        st.subheader("Hasil Evaluasi Model GLM")
        st.write(pd.DataFrame({
            "Metrik": ["Mean Squared Error", "Root Mean Squared Error", "Mean Absolute Error", "Mean Absolute Percentage Error", "R-squared", "Adjusted R-squared"],
    ...
    # Model ARIMA
        ...
        try:
            ...
            model_fit = model.fit()
            st.info("Model ARIMA terlatih dengan order (0,1,3)")
            ...
            st.write(pd.DataFrame({
                "Metrik": ["Mean Squared Error", "Root Mean Squared Error", "Mean Absolute Error",
            ...
            with st.expander("Ringkasan Model ARIMA"):
                st.code(model_fit.summary().as_text(), language='text')
...
# Halaman prediksi
    ...
    if model_type == "GLM":
        ...
        ts_data = ts_data.astype(float).dropna()
        try:
            model = ARIMA(ts_data, order=(0, 1, 3))
            model_fit = model.fit()
            st.success("Model ARIMA berhasil dilatih dengan order (0, 1, 3)")
            # Input jumlah hari untuk prediksi ke depan
            st.subheader("Prediksi Harga Saham ke Depan")
            ...
            ax.plot(df_forecast.index, df_forecast["Prediksi_ARIMA"], label="Prediksi ke Depan", linestyle='--',
                    color="orange")
            ax.set_title("Prediksi Harga Saham Menggunakan ARIMA (Manual Order)")
